...
static NTSTATUS NTAPI RtlFindLdrpHandleTlsDataOld() {
	...
	SEARCH_CONTEXT SearchContext{
		SearchContext.SearchPattern = const_cast<LPBYTE>(static_cast<const BYTE*>(Feature)),
		SearchContext.PatternSize = Size - 1 };
...
static NTSTATUS NTAPI RtlFindLdrpReleaseTlsEntry() {
	...
	SEARCH_CONTEXT SearchContext{ SearchContext.SearchPattern = const_cast<LPBYTE>(static_cast<const BYTE*>(Feature)) ,SearchContext.PatternSize = Size - 1 };
	if (!NT_SUCCESS(RtlFindMemoryBlockFromModuleSection(static_cast<HMODULE>(MmpGlobalDataPtr->MmpBaseAddressIndex->NtdllLdrEntry->DllBase), ".text", &SearchContext)))
		return STATUS_NOT_SUPPORTED;
