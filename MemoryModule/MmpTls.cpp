...
PMMP_TLSP_RECORD MmpFindTlspRecordLockHeld() {
    PLIST_ENTRY entry = GetMmpGlobalDataPtr()->MmpTls->MmpThreadLocalStoragePointer.Flink;
    PTEB teb = NtCurrentTeb();

    while (entry != &GetMmpGlobalDataPtr()->MmpTls->MmpThreadLocalStoragePointer) {
...
DWORD MmpAllocateTlsLockHeld() {
    ...
    //
    EnterCriticalSection(&GetMmpGlobalDataPtr()->MmpTls->MmpTlspLock);
    ...
    if (!!record) {
        LeaveCriticalSection(&GetMmpGlobalDataPtr()->MmpTls->MmpTlspLock);
        goto __skip_tls;
    }
    ...
    if (record) {
        ...
        if (record->TlspMmpBlock) {
            ...
            InsertTailList(&GetMmpGlobalDataPtr()->MmpTls->MmpThreadLocalStoragePointer, &record->InMmpThreadLocalStoragePointer);
            success = true;
        }
        else {
    ...
    //
    if (success) {
        RtlAcquireSRWLockShared(&GetMmpGlobalDataPtr()->MmpTls->MmpTlsListLock);

        auto ThreadLocalStoragePointer = (PVOID*)NtCurrentTeb()->ThreadLocalStoragePointer;
        PLIST_ENTRY entry = GetMmpGlobalDataPtr()->MmpTls->MmpTlsList.Flink;
        while (entry != &GetMmpGlobalDataPtr()->MmpTls->MmpTlsList) {
        ...
        RtlReleaseSRWLockShared(&GetMmpGlobalDataPtr()->MmpTls->MmpTlsListLock);
    }
    ...
    InterlockedIncrement(&GetMmpGlobalDataPtr()->MmpTls->MmpActiveThreadCount);
...
VOID NTAPI HookRtlUserThreadStart(
    ...
    return GetMmpGlobalDataPtr()->MmpTls->Hooks.OriginRtlUserThreadStart(MmpUserThreadStart, &Context);
}
#else
VOID
__declspec(naked)
HookRtlUserThreadStart(
    ...
    __asm {
        ...
        // GetMmpGlobalDataPtr()->MmpTls->Hooks.OriginRtlUserThreadStart(MmpUserThreadStart, &Context);
        mov ecx, GetMmpGlobalDataPtr();
...
VOID NTAPI HookLdrShutdownThread(VOID) {
    ...
    if (record) {
        ...
        --GetMmpGlobalDataPtr()->MmpTls->MmpActiveThreadCount;
    }

    assert(0 < (int)GetMmpGlobalDataPtr()->MmpTls->MmpActiveThreadCount);
    ...
    if (record) {
        ...
        if (GetMmpGlobalDataPtr()->MmpTls->MmpTlsList.Flink != &GetMmpGlobalDataPtr()->MmpTls->MmpTlsList) {
    ...
    //
    GetMmpGlobalDataPtr()->MmpTls->Hooks.OriginLdrShutdownThread();
}
...
NTSTATUS NTAPI HookNtSetInformationProcess(
    ...
    if (ProcessInformationClass != ProcessTlsInformation) {
        return GetMmpGlobalDataPtr()->MmpTls->Hooks.OriginNtSetInformationProcess(
            ProcessHandle,
    ...
    do {
        ...
        status = GetMmpGlobalDataPtr()->MmpTls->Hooks.OriginNtSetInformationProcess(
            hProcess,
            ProcessInformationClass,
            Tls,
            TlsLength
...
NTSTATUS NTAPI MmpAcquireTlsIndex(_Out_ PULONG TlsIndex) {
    ...
    *TlsIndex = -1;
    ...
    ULONG Index = RtlFindClearBitsAndSet(&GetMmpGlobalDataPtr()->MmpTls->MmpTlsBitmap, 1, 0);
    if (Index != -1) {
        *TlsIndex = Index;
...
NTSTATUS NTAPI MmpAllocateTlsEntry(
    ...
    RtlAcquireSRWLockExclusive(&GetMmpGlobalDataPtr()->MmpTls->MmpTlsListLock);
    InsertTailList(&GetMmpGlobalDataPtr()->MmpTls->MmpTlsList, &Entry->TlsEntryLinks);
    RtlReleaseSRWLockExclusive(&GetMmpGlobalDataPtr()->MmpTls->MmpTlsListLock);
...
NTSTATUS NTAPI MmpReleaseTlsEntry(_In_ PLDR_DATA_TABLE_ENTRY lpModuleEntry) {
    ...
    for (auto entry = GetMmpGlobalDataPtr()->MmpTls->MmpTlsList.Flink; entry != &GetMmpGlobalDataPtr()->MmpTls->MmpTlsList; entry = entry->Flink) {
        auto p = CONTAINING_RECORD(entry, TLS_ENTRY, TlsEntryLinks);
        if (p->ModuleEntry == lpModuleEntry) {
            RemoveEntryList(&p->TlsEntryLinks);
            RtlClearBit(&GetMmpGlobalDataPtr()->MmpTls->MmpTlsBitmap, p->TlsDirectory.Characteristics);
...
NTSTATUS NTAPI MmpHandleTlsData(_In_ PLDR_DATA_TABLE_ENTRY lpModuleEntry) {
    ...
    auto ThreadCount = GetMmpGlobalDataPtr()->MmpTls->MmpActiveThreadCount;
    auto success = true;
    ...
    if (ThreadCount) {
        EnterCriticalSection(&GetMmpGlobalDataPtr()->MmpTls->MmpTlspLock);
        GetMmpGlobalDataPtr()->MmpTls->MmpActiveThreadCount -= ThreadCount;
        assert(GetMmpGlobalDataPtr()->MmpTls->MmpActiveThreadCount > 0);
        LeaveCriticalSection(&GetMmpGlobalDataPtr()->MmpTls->MmpTlspLock);
    }
...
BOOL NTAPI MmpTlsInitialize() {
    ...
    //
    GetMmpGlobalDataPtr()->MmpTls->MmpActiveThreadCount = MmpGetThreadCount();

    //
    // Initialize tlsp
    //
    InitializeCriticalSection(&GetMmpGlobalDataPtr()->MmpTls->MmpTlspLock);
    InitializeListHead(&GetMmpGlobalDataPtr()->MmpTls->MmpThreadLocalStoragePointer);

    //
    // Initialize tls list
    //
    InitializeListHead(&GetMmpGlobalDataPtr()->MmpTls->MmpTlsList);
    RtlInitializeSRWLock(&GetMmpGlobalDataPtr()->MmpTls->MmpTlsListLock);
    ...
    RtlInitializeBitMap(&GetMmpGlobalDataPtr()->MmpTls->MmpTlsBitmap, buffer, MMP_MAXIMUM_TLS_INDEX);
    ...
    GetMmpGlobalDataPtr()->MmpTls->Hooks.OriginLdrShutdownThread = LdrShutdownThread;
    GetMmpGlobalDataPtr()->MmpTls->Hooks.OriginNtSetInformationProcess = NtSetInformationProcess;
    GetMmpGlobalDataPtr()->MmpTls->Hooks.OriginRtlUserThreadStart = (decltype(&RtlUserThreadStart))GetProcAddress((HMODULE)GetMmpGlobalDataPtr()->MmpBaseAddressIndex->NtdllLdrEntry->DllBase, "RtlUserThreadStart");

    DetourTransactionBegin();
    DetourUpdateThread(NtCurrentThread());
    DetourAttach((PVOID*)&GetMmpGlobalDataPtr()->MmpTls->Hooks.OriginLdrShutdownThread, HookLdrShutdownThread);
    DetourAttach((PVOID*)&GetMmpGlobalDataPtr()->MmpTls->Hooks.OriginNtSetInformationProcess, HookNtSetInformationProcess);
    DetourAttach((PVOID*)&GetMmpGlobalDataPtr()->MmpTls->Hooks.OriginRtlUserThreadStart, HookRtlUserThreadStart);
...
VOID NTAPI MmpTlsCleanup() {
    ...
    DetourTransactionBegin();
    DetourUpdateThread(NtCurrentThread());
    DetourDetach((PVOID*)&GetMmpGlobalDataPtr()->MmpTls->Hooks.OriginLdrShutdownThread, HookLdrShutdownThread);
    DetourDetach((PVOID*)&GetMmpGlobalDataPtr()->MmpTls->Hooks.OriginNtSetInformationProcess, HookNtSetInformationProcess);
    DetourDetach((PVOID*)&GetMmpGlobalDataPtr()->MmpTls->Hooks.OriginRtlUserThreadStart, HookRtlUserThreadStart);
    DetourTransactionCommit();
