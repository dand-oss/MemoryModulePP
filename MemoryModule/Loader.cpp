	...
	__try {
		*BaseAddress = nullptr;
		...
		if (!RtlIsValidImageBuffer(BufferAddress, &BufferSize) && !(dwFlags & LOAD_FLAGS_PASS_IMAGE_CHECK)) {
			...
			std::wcerr << std::format(L"Invalid image buffer for {} (Status: {:#x})", DllName ? DllName : L"null", status) << std::endl;
		}

		if (MmpGlobalDataPtr == nullptr) {
			status = STATUS_INVALID_PARAMETER;
			std::wcerr << std::format(L"Invalid MmpGlobalDataPtr for {} (Status: {:#x})", DllName ? DllName : L"null", status) << std::endl;
		}
	}
	__except (EXCEPTION_EXECUTE_HANDLER) {
		status = GetExceptionCode();
		std::wcerr << std::format(L"Exception in initial checks for {} (Status: {:#x})", DllName ? DllName : L"null", status) << std::endl;
	}
