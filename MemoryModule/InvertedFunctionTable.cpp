...
static NTSTATUS RtlProtectMrdata(_In_ ULONG Protect) {
	...
	if (!MrdataBase) {
		...
		status = NtQueryVirtualMemory(GetCurrentProcess(), GetMmpGlobalDataPtr()->MmpInvertedFunctionTable->LdrpInvertedFunctionTable, MemoryBasicInformation, &mbi, sizeof(mbi), nullptr);
...
NTSTATUS NTAPI RtlInsertInvertedFunctionTable(
	...
	auto table = PRTL_INVERTED_FUNCTION_TABLE(GetMmpGlobalDataPtr()->MmpInvertedFunctionTable->LdrpInvertedFunctionTable);
